...
const BattleInterface: React.FC<BattleInterfaceProps> = ({ playerTeam, opponentTeam, onBattleEnd }) => {
  ...
  const calculateDamage = (attacker: BattlePokemon, defender: BattlePokemon, move: Attack) => {
    ...
    for (const defenderType of defender.types) {
        typeMultiplier *= typeChart[move.type]?.[defenderType] ?? 1;
    }
    ...
    if (typeMultiplier < 1 && typeMultiplier > 0) addLog("It's not very effective...");
    if (typeMultiplier === 0) addLog("It had no effect!");
        ...
        if (fainted) {
            ...
            if (remaining.length === 0) {
              ...
              battleEnded = true;
                ...
                if (turn.actor === 'player') { // Opponent fainted
                    ...
                    return; // End turn processing to wait for player switch
                }
            }
        }
  ...
  const handlePlayerSwitch = (pokemon: BattlePokemon) => {
    ...
    if (isForcedSwitch) {
        ...
        processTurn({ type: 'switch', pokemonId: pokemon.id });
    }
  };
    ...
    <div className={`absolute w-1/2 md:w-1/3 max-w-xs ${isOpponent ? 'top-4 right-4 text-right' : 'bottom-[210px] left-4'}`}>
        ...
        <div className={`relative h-32 md:h-40 ${isOpponent ? 'mr-4' : 'ml-4'}`}>
            <img 
                ...
                alt={pokemon.name} 
                className={`absolute inset-0 h-full w-full object-contain drop-shadow-lg transition-transform duration-300
                    ${((isOpponent && opponentAnim.damage) || (!isOpponent && playerAnim.damage)) ? 'animate-pulse-fast' : ''}
                    ${((isOpponent && opponentAnim.attack) || (!isOpponent && playerAnim.attack)) ? (isOpponent ? 'translate-x-[-20px]' : 'translate-x-[20px]') : ''}
  ...
  const renderActionPanel = () => {
                ...
                {(playerActive.attacks).map(attack => (
                    <Button key={attack.name} onClick={() => processTurn({ type: 'attack', move: attack })} className="flex-col !gap-0 !items-start !text-left p-2">
                        ...
                        <div className="text-xs opacity-80 flex items-center gap-2">
                           ...
                           <span>{attack.power} Pwr</span>
                        </div>
                    </Button>
                ))}
                 <Button variant="ghost" onClick={() => setActionView('main')} className="col-span-2">
    ...
    return (
        <div className="grid grid-cols-2 gap-4 h-full">
            <Button onClick={() => setActionView('fight')} className="h-full text-2xl">Fight</Button>
            <Button variant="secondary" onClick={() => setActionView('pokemon')} className="h-full text-2xl">Pokémon</Button>
        </div>
    );
  };

  return (
    <div className="bg-slate-900/50 rounded-lg border-2 border-slate-700 relative" style={{ height: '75vh', minHeight: '500px' }}>
      ...
      <Modal isOpen={!!outcome} onClose={() => {}} title={outcome === 'victory' ? 'Victory!' : 'Defeat!'}>
        ...
        <Button onClick={onBattleEnd} className="w-full">
